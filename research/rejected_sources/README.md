...
**Two large files referenced below (`preprocessed_content.csv`,
86.4MB, and `environmental_data_clean.csv`, 87.0MB) have been removed
from the repository itself** — both were slowing down every clone/pull
of the repo for the whole team, and the reasoning below is fully
self-contained without needing the raw files present. If you need the
original data, it is the shelved NLP-derived ESG dataset described in
`docs/RESEARCH_LOG.md` §3–4; regenerate or re-source it from there if
required.
